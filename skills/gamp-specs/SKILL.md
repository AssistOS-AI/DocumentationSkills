...
- Structure `DS003-main-behavior.md` specially while retaining `Introduction`, `Core Content`, and `Conclusion` as its only top-level content sections. Inside `Core Content`, create one `###` chapter for every accepted Main Behavior component, using the ordered component names from the detector handoff.
