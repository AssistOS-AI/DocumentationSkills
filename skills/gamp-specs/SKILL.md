...
- Put a required `Documentation Map` section on `docs/index.html` after the project overview and before detailed technical content. Render the map as a responsive, column-oriented table. Each column header must be the main documentation page or subject area itself, represented by a real link to that page; do not add generic headers such as `Order`, `Main page`, `Purpose`, or `Subpages`.
- Put that main page's immediate subpages in the body cells directly below its column header. Every non-empty body cell must contain exactly one real `<a>` link styled as a compact button. When columns have different numbers of immediate children, leave the remaining cells empty and non-interactive; do not add placeholder labels or invent pages.
- Derive main-page and subpage relationships from the actual documentation architecture rather than grouping independent pages under a broad topic. In a skill-catalog repository, treat every per-skill HTML page as a main page and label its map shortcut with the canonical skill name followed by `skill`, for example `gamp-specs skill`. Treat the specification matrix or specification entry page as a main page, but never enumerate individual DS or specification files in the Documentation Map. When no eligible subpages exist, omit the table body instead of generating empty rows.
- Style both main-page and subpage links as shortcuts while keeping the visual distinction between column headers and child cells. Do not use non-navigating `<button>` elements, JavaScript-only navigation, a decorative card grid, or plain unlinked page labels. Every shortcut must have meaningful link text, a visible keyboard focus state, and a target that exists.
- Preserve semantic table markup without visually drawing a rigid spreadsheet grid. Remove per-cell boxes and strong row or column borders from the Documentation Map; use whitespace, alignment, a restrained shared background, and subtle shortcut styling to communicate the columns. Empty cells must remain visually empty.
- Keep the hierarchy selective: include every primary page as one column header and every eligible immediate child page exactly once beneath its parent, but do not flatten deeper leaf pages into the home page. Individual DS and specification files are never eligible map children.
- Follow the table with concise prose that explains the purpose of the main pages and tells a new reader where to start and how to proceed through concepts, usage, operations, reference material, and specifications. Use actual page names, keep purpose and reading-order metadata out of the table, and do not introduce undefined terminology in the reading flow.
...
- Verify that `docs/index.html` contains the required column-oriented Documentation Map table, that every main page appears exactly once as a linked column header, that every eligible immediate subpage appears exactly once beneath its real parent, and that every shortcut resolves. Verify that per-skill pages in a skill catalog are main pages labeled `<canonical-skill-name> skill`, that individual specification files are absent, and that a table body is omitted when there are no eligible subpages. When a body is present, verify that non-empty cells contain only one linked shortcut and remaining alignment cells are empty. Verify that the purpose and recommended-reading prose below the table agrees with the surrounding documentation and header hierarchy.
...
- Confirm the home-page Documentation Map is a responsive, column-oriented table rather than a card grid: each linked column header is a main page, each non-empty cell beneath it is one linked eligible immediate subpage, and there are no generic header labels or purpose/order metadata columns. In skill catalogs, confirm every per-skill page is a main column with the required `skill` suffix, the specification entry appears once, and individual specification files do not appear. Confirm that semantic table cells are not rendered as a rigid bordered grid, that whitespace and alignment preserve the relationships, that the prose below the map explains page purpose and reading flow, and that the map does not compete with the header as a second primary navigation system.
